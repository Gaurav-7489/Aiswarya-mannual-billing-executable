...
export async function GET() {
  ...
  const summary = db.prepare(`
    SELECT
      COALESCE(SUM(CASE WHEN status = 'FINALIZED' THEN grand_total_minor ELSE 0 END), 0) AS todaySalesMinor,
      COUNT(CASE WHEN status = 'FINALIZED' THEN 1 END) AS todayInvoiceCount,
  ...
  const counts = db.prepare(`
    SELECT
      ...
      (SELECT COUNT(*) FROM notifications WHERE status = 'FAILED') AS failedNotificationCount
  ...
  const topCustomers = db.prepare(`
    SELECT c.id, c.name, c.code,
      ...
      COALESCE(SUM(CASE WHEN i.status = 'FINALIZED' THEN i.grand_total_minor ELSE 0 END), 0) AS totalSalesMinor
    FROM customers c
    LEFT JOIN invoices i ON i.customer_id = c.id
    WHERE c.is_active = 1
    GROUP BY c.id
    ORDER BY totalSalesMinor DESC, invoiceCount DESC
    LIMIT 5
  `).all()
  ...
  return NextResponse.json({
    today,
    summary: {
      todaySalesMinor: Number(summary.todaySalesMinor ?? 0),
      todayInvoiceCount: Number(summary.todayInvoiceCount ?? 0),
    ...
    counts: {
      ...
      failedNotificationCount: Number(counts.failedNotificationCount ?? 0),
    },
    recentInvoices,
    topCustomers,
  })
