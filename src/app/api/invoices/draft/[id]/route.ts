...
export async function POST(request: Request, { params }: Params) {
  ...
  try {
    ...
    const result = finalizeDraft(id, body.gstMode === 'INTER' ? 'INTER' : 'INTRA')
    queueInvoiceNotifications(result.id)
    return NextResponse.json({ ok: true, invoice: result })
  } catch (error) {
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : 'Unable to finalize draft.' }, { status: 400 })
  }
}
