...
export default function InvoiceWorkspace() {
    ...
    async function restore() {
        ...
        if (!cancelled && latest?.id) {
          ...
          if (detail.ok && detail.invoice?.status === 'DRAFT') {
            const customer: Customer = {
              id: String(detail.invoice.customer_id), name: String(detail.invoice.customer_name), code: String(detail.invoice.customer_code),
              phone: detail.invoice.customer_phone ?? null, city: detail.invoice.customer_city ?? null, gstin: detail.invoice.customer_gstin ?? null,
            }
            const restoredItems: LineItem[] = (detail.items ?? []).map((item: Record<string, unknown>) => ({
              id: String(item.product_id ?? item.id), name: String(item.product_name), code: String(item.product_code),
              packSize: item.pack_size ? String(item.pack_size) : null, unit: String(item.unit), rateMinor: Number(item.rate_minor),
              gstRateBps: Number(item.gst_rate_bps), hsnCode: item.hsn_code ? String(item.hsn_code) : null, qty: Number(item.quantity),
            }))
            setSelectedCustomer(customer); setItems(restoredItems); setDiscount(Number(detail.invoice.discount_minor ?? 0) / 100); setDraftId(latest.id)
            setDraftStatus('Restored from local database')
      ...
      try {
        ...
        if (raw && !cancelled) {
          ...
          setSelectedCustomer(state.customer); setItems(state.items ?? []); setDiscount(state.discount ?? 0); setDraftId(state.draftId ?? null)
          setDraftStatus('Recovered from this computer')
        }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        const data = await response.json(); setCustomerResults(data.customers ?? [])
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        const data = await response.json(); setProductResults(data.products ?? [])
  ...
  }, [selectedCustomer, items, discount, draftId, savedInvoice])
  ...
  async function autosaveDraft() {
    ...
    try {
      const response = await fetch('/api/invoices/draft', {
        ...
        body: JSON.stringify({ id: draftId ?? undefined, customerId: selectedCustomer.id, items: items.map(item => ({ productId: item.id, quantity: item.qty, rateMinor: item.rateMinor })), discountMinor, gstMode: 'INTRA' }),
      })
      const data = await response.json()
      if (!response.ok || !data.ok) throw new Error(data.error || 'Unable to save draft.')
      setDraftId(data.draft.id); setDraftStatus('Draft saved automatically');
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ customer: selectedCustomer, items, discount, draftId: data.draft.id }))
    } catch (err) { setDraftStatus('Saved on this computer'); setError(err instanceof Error ? err.message : 'Unable to save draft.') }
    finally { setSaving(false) }
  }

  const addItem = (product: Product) => {
    setItems(current => current.some(item => item.id === product.id) ? current.map(item => item.id === product.id ? { ...item, qty: item.qty + 1 } : item) : [...current, { ...product, qty: 1 }])
    setProductQuery(''); setSavedInvoice(null); setError(null)
  }
  ...
  const clearInvoice = () => {
    setSelectedCustomer(null); setCustomerQuery(''); setProductQuery(''); setItems([]); setDiscount(0); setDraftId(null); setSavedInvoice(null); setDraftStatus(''); setError(null); localStorage.removeItem(STORAGE_KEY)
  }
  ...
  return (
    <main className="invoice-workspace">
      <header className="invoice-topbar">
        ...
        <div className="invoice-top-actions"><span className="local-status">LOCAL · DATABASE</span><span className={`draft-status ${draftStatus.includes('saved') || draftStatus.includes('Restored') || draftStatus.includes('Recovered') ? 'good' : ''}`}>{draftStatus || 'Ready'}</span></div>
      </header>
        ...
        <div className="invoice-main">
          ...
          <section className="invoice-card">
            ...
            <div className="invoice-table">
              ...
              {items.length === 0 ? <div className="table-empty">No products added yet. Search above to add the first item.</div> : items.map(item => <div className="table-row" key={item.id}><div><strong>{item.name}</strong><small>{item.code}</small></div><span>{item.packSize || '—'}</span><input aria-label={`Quantity for ${item.name}`} type="number" min="1" value={item.qty} onChange={e => updateQuantity(item.id, e.target.value)} /><span>{money(item.rateMinor)}</span><strong>{money(item.rateMinor * item.qty)}</strong><button className="remove-button" onClick={() => removeItem(item.id)}>Remove</button></div>)}
            </div>
          </section>
        </div>
        <aside className="invoice-summary invoice-card">
          <div className="invoice-section-title"><b>03</b><div><h2>Review & Save</h2><p>Your work is autosaved while you build the invoice.</p></div></div>
          ...
          <small className="summary-note">* GST is calculated from each product&apos;s configured GST rate when finalized.</small>
          {error && <div className="save-error" role="alert"><strong>Attention</strong><span>{error}</span></div>}
          {savedInvoice && <div className="save-success" role="status"><strong>Invoice finalized</strong><span>{savedInvoice} · Stored in the local database.</span></div>}
          <button className="save-button" disabled={!selectedCustomer || items.length === 0 || finalizing} onClick={finalize}>{finalizing ? 'Finalizing…' : savedInvoice ? 'Start New Invoice' : 'Finalize & Save Invoice'}</button>
