...
const items = [
  { href:'/dashboard', label:'Home', short:'⌂' },
  ...
  return <>
    <header className="app-nav">
      <Link href="/dashboard" className="app-brand" onClick={()=>setOpen(false)}>
        <span className="app-brand-mark">A</span><span><strong>Aiswarya</strong><small>Food Products</small></span>
      </Link>
    ...
    <nav className="mobile-bottom-nav" aria-label="Quick navigation">{items.slice(0,6).map(item=><Link key={item.href} href={item.href} className={pathname===item.href?'active':''}><span>{item.short}</span><small>{item.label}</small></Link>)}</nav>
  </>
}
