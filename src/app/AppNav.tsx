...
export default function AppNav(){
 ...
 return <>
  <aside className="app-nav" aria-label="Aiswarya navigation">
    <Link href="/dashboard" className="app-brand" onClick={()=>setOpen(false)}><span className="app-brand-mark">A</span><span><strong>Aiswarya</strong><small>FOOD PRODUCTS</small></span></Link>
    ...
    <Link href="/invoice" className="nav-counter-button"><Icon name="plus"/><span>New Bill</span></Link>
  </aside>
  <header className="mobile-app-bar"><Link href="/dashboard" className="app-brand"><span className="app-brand-mark">A</span><span><strong>Aiswarya</strong><small>FOOD PRODUCTS</small></span></Link><button className="nav-menu-button" aria-label="Open navigation" aria-expanded={open} onClick={()=>setOpen(v=>!v)}>☰</button></header>
